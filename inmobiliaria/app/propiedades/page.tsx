...
export default async function Propiedades({ searchParams }: { searchParams: Promise<{ tipo?: string; pv?: string; pa?: string; pt?: string }> }) {
  ...
  const filtros = <nav className={styles.filterOptions} aria-label="Filtrar por tipo">
    <Link href="/propiedades" aria-current={!tipo ? "true" : undefined} className={`${styles.filterLink} ${!tipo ? styles.filterActive : ""}`}>Todas <span>{totalGeneral}</span></Link>
    {tiposDisponibles.map(t => <Link key={t} href={`/propiedades?tipo=${encodeURIComponent(t)}`} aria-current={tipo === t ? "true" : undefined} className={`${styles.filterLink} ${tipo === t ? styles.filterActive : ""}`}>{t} <span>{conteoPorTipo[t]}</span></Link>)}
  </nav>

  return <main className={styles.page}>
    <section className={styles.hero} aria-labelledby="propiedades-title">
      <div className={`${styles.container} ${styles.heroLayout}`}>
        <div><p className={styles.eyebrow}>Necochea y alrededores</p><h1 id="propiedades-title">Propiedades</h1></div>
        <div className={styles.heroSummary}><p>Venta, alquiler y temporada.</p><span>{totalFiltrado} {totalFiltrado === 1 ? "propiedad disponible" : "propiedades disponibles"}</span></div>
      </div>
    </section>
    ...
    <section className={styles.catalogue}><div className={styles.container}>
      ...
      <div className={styles.sections}>{secciones.map((s) => s.count > 0 && <section key={s.clave} id={s.clave.replace(/\s+/g, "-").toLowerCase()} className={styles.propertySection}>
        <header className={styles.sectionHeader}><div><div><h2>{s.titulo}</h2><span>{s.subtitulo}</span></div></div><span>{s.count} {s.count === 1 ? "propiedad" : "propiedades"}</span></header>
        <div className={styles.grid}>{s.items.map((p, index) => <Reveal key={p.id} delay={(index % 3) * 90}><Link href={`/propiedades/${p.id}`} className={styles.card}>
          <div className={styles.imageWrap}>{p.imagen_url ? <img src={p.imagen_url} alt={p.titulo} loading="lazy" decoding="async" /> : <div className={styles.noImage}>Sin imagen disponible</div>}<div className={styles.badges}>{esNueva(p.created_at) && <span className={styles.newBadge}>Recién publicada</span>}</div></div>
          <div className={styles.cardBody}><div className={styles.cardTopline}><span className={styles.propertyType}>{p.tipo} · {s.clave}</span><p className={styles.price}>{formatearPrecio(p.precio, p.moneda)}</p></div><h3>{p.titulo}</h3><p className={styles.location}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.5"/></svg><span>{p.ubicacion}</span></p><div className={styles.cardFooter}><span><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M4 9V4h5M15 4h5v5M20 15v5h-5M9 20H4v-5"/></svg>{p.superficie} m²</span><span className={styles.cta}>Ver ficha</span></div></div>
        </Link></Reveal>)}</div>
    ...
    <footer className={styles.footer}><div className={styles.container}><Link href="/" className={styles.footerBrand}><small>Inmobiliaria</small>Liliana Cirigliano</Link><nav aria-label="Navegación al pie"><Link href="/">Inicio</Link><Link href="/nosotros">Nosotros</Link><Link href="/contacto">Contacto</Link></nav></div><div className={`${styles.container} ${styles.footerBottom}`}><p>© {new Date().getFullYear()} Inmobiliaria Liliana Cirigliano.</p><span>Necochea, Buenos Aires</span></div></footer>
  </main>
}
